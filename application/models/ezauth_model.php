...
//	EzAuth v 0.6
//	This software is open source, HOWEVER it may not be modified and resold without permission from BizWidgets! Thankyou
//	Hand Crafted using CodeIgniter by Daniel Errante
//	daniel@bizwidgets.biz
//	http://bizwidgets.biz/


//Modified by Tom Kimani
//FlexiPay V 1.0
//Date May 2013
...
var $user		 			=	array();	//	user info in database
	...
	function login($un = null, $pw = null, $give_new_key = false, $cookie_hash = '', $get_all_user_info = true) {
		...
		$pw = (empty($pw)) ? $this->input->post('passcode') : $pw;
		...
		// ----If we have a cookie hash,Use It else use the Mobile/passcode Pair -------------------
	...
	*/
	...
	//	v 0.4
	//new in 0.6: activation keys are generated by user information and a timestamp, and are only enabled after a user verifies their phone_number
	...
	function register($inp, $verify = true, $remember_user = false, $login_after_register = false) {
		//TODO
		//1. Confirm that phone Number and account type-id are sent by the api
		...
		$mobile = $inp['ez_users']['mobile_number'];
		$pw = $inp['passcode'];
...
/*
		...
		$account_type_id = $inp['account_id'];
		...
		//3. If the account is for Merchant/Agent Add Necessary Details
		//Merchant - Details
		if($account_type_id == 1){
			$inp['merchant_description']['user_id'] = $user_id;
			$this->db->insert('merchant_description', $inp['merchant_description']);
		}

		//Agent - Details
		else if($account_type_id == 2){
			$inp['agent_description']['user_id'] = $user_id;
			$this->db->insert('agent_description', $inp['agent_description']);
		}


		//	make very unique cookie hash for auto-login
		$salt = microtime();
		$cookie_hash = $this->_add_salt($user_id.$inp['passcode'].$email, $salt);
		...
		if ($verify == true) {
			...
			//$md5_vcode = $this->_add_salt($v_code); // TODO :: Find out why they are adding salt to verification code ??
			$inp3['activation_code'] = $v_code;
		} 
		$this->db->insert('ez_auth', $inp3);		
		


		/* Access-Keys for various Pages in the site*/
